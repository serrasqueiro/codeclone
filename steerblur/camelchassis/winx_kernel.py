...
class PSECURITY_DESCRIPTOR(PLOCAL):
    ...
    def get_owner(self, system_name=None):
        if not self or not self.pOwner:
            raise ValueError('NULL pointer access')
        trip = look_up_account_sid(self.pOwner, system_name)
        return trip
...
if __name__ == '__main__':
    ...
    if owner_domain:
        compl_name = '{}\\{}'.format(owner_domain, owner_name)
    else:
        compl_name = owner_name
    print("Path : {}".format(filename))
    print("Owner: {} ({})".format(compl_name, owner_sid_type), "as:", owner_name)
